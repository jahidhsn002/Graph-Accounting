...
<table class="table table-bordered">
	<thead>
	  <tr>
	  	<th v-if="this.offer.conditions.length<=0"><i class="glyphicon glyphicon-trash"></i></th>
		<th>Description</th>
		<th>Brand/Origin</th>
	...
	<tbody>
	  <tr v-for="(point, item) in offer.items">
	  	<td v-if="this.offer.conditions.length<=0">
	  		<button class="btn btn-danger btn-sm " v-on:click.prevent="offer.items.splice(point,1)"><i class="glyphicon glyphicon-trash"></i></button>
	  	</td>
		<td>
			<div :class="errors['offer.items.'+point+'.description']?'has-error btn-group bootstrap-select':'btn-group bootstrap-select'">
				...
				<div class="dropdown-menu open">
			        <ul class="dropdown-menu inner">
			          <li v-for="dbproduct in products | filterBy item.description in 'description'">
			            <a href="#" v-on:click.prevent="addItem(point, offer.items[point].qty, offer.items[point].discount, dbproduct)">
	  ...
	  <tr v-if="this.offer.conditions.length<=0">
